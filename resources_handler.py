...
RESOURCE_TYPES = {
    "personnel": {
        ...
        "label": "人员队伍",
        "id_prefix": "P",
        "fields": ["name", "type", "team", "phone", "skills", "status", "location"],
        "required": ["name", "phone"],
        "excel_columns": ["姓名", "类型", "队伍", "联系电话", "技能(逗号分隔)", "状态", "所在位置"],
    },
    "materials": {
        ...
        "label": "物资装备",
        "id_prefix": "M",
        "fields": ["name", "category", "quantity", "unit", "location", "status", "specs", "manager", "manager_phone"],
        "required": ["name", "quantity"],
        "excel_columns": ["物资名称", "类别", "数量", "单位", "存放位置", "状态", "规格说明", "管理员", "联系电话"],
    },
    "facilities": {
        ...
        "label": "场所设施",
        "id_prefix": "F",
        "fields": ["name", "type", "capacity", "address", "lat", "lng", "contact", "phone", "status"],
        "required": ["name", "address"],
        "excel_columns": ["设施名称", "类型", "容纳人数", "详细地址", "纬度", "经度", "负责人", "联系电话", "状态"],
    },
    "vehicles": {
        ...
        "label": "车辆运力",
        "id_prefix": "V",
        "fields": ["plate_number", "type", "model", "driver", "driver_phone", "status", "location"],
        "required": ["plate_number", "type"],
        "excel_columns": ["车牌号", "车辆类型", "车辆型号", "驾驶员", "联系电话", "状态", "存放位置"],
    },
}
...
FIELD_MAP = {
    ...
    "materials": {
        ...
        "存放位置": "location", "状态": "status", "规格说明": "specs",
...
def _get_example_row(resource_type: str) -> list:
    if resource_type == "personnel":
        return ["张三", "救援队员", "区应急救援队", "13800138000", "水上救援,医疗急救", "待命", "XX消防站"]
    elif resource_type == "materials":
        return ["移动水泵", "排水设备", 10, "台", "XX仓库", "可用", "功率200kW", "李四", "13900139000"]
    elif resource_type == "facilities":
        return ["XX应急避难所", "避难所", 500, "XX区XX街道XX路XX号", 23.12, 113.26, "王五", "13700137000", "备用"]
    elif resource_type == "vehicles":
        return ["粤A12345", "抢险车", "东风应急车", "赵六", "13600136000", "可用", "XX停车场"]
    return []
